...
export default function UserForm({ onUserAdded }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      formData.domains.forEach((domain, index) => {
        formDataToSend.append(`domains[${index}][domainId]`, domain.domainId)
        formDataToSend.append(`domains[${index}][userRole]`, domain.userRole)
      })
  ...
  return (
    <div style={{ maxWidth: '600px', margin: '2rem auto' }}>
      <h2 style={{ marginBottom: '2rem', textAlign: 'center' }}>Add New User</h2>
      
      <form onSubmit={handleSubmit} style={{ background: 'white', padding: '2rem', borderRadius: '0.5rem', border: '1px solid #ddd' }}>
        {/* Username */}
          ...
          {formData.domains.map((domain, index) => (
            <div key={index} style={{ display: 'flex', gap: '0.5rem', marginBottom: '1rem', alignItems: 'center' }}>
              <select
                ...
                onChange={(e) => handleDomainChange(index, 'domainId', e.target.value)}
                required
                style={{
                  flex: 1,
                  padding: '0.75rem',
              ...
              <select
                ...
                onChange={(e) => handleDomainChange(index, 'userRole', e.target.value)}
                required
                style={{
                  flex: 1,
                  padding: '0.75rem',
